...
export class CypressScriptGenerator {
  ...
  // private pageObjects: Map<string, PageObjectModel> = new Map(); // Reserved for future use
  ...
  async generateScript(
    ...
    _explorationResult: ExplorationResult,
    ...
    const metadata = {
      ...
      pageObjects: this.options.usePageObjects ? this.extractPageObjects(_pageAnalysis) : undefined,
  ...
  private async buildScriptContent(
    ...
    content += this.generateImports(linkedData, _pageAnalysis);
    ...
    content += this.generateDescribeBlock(testCase, linkedData, _explorationResult, _pageAnalysis);

    // Add helper functions if needed
    content += this.generateHelperFunctions(linkedData, _pageAnalysis);
 ...
 * Scenario: ${testCase.scenario_name || 'Auto-generated test case'}
 * 
...
`;
  ...
  private generateDescribeBlock(
    ...
    if (this.options.includeSetup) {
      content += this.generateSetup(_pageAnalysis);
    }

    // Add main test
    content += this.generateMainTest(testCase, linkedData, _explorationResult, _pageAnalysis);
  ...
  private generateMainTest(
    ...
    const testDescription = testCase.scenario_name || 'should execute test scenario successfully';
    let test = `\n  it('${testDescription}', () => {\n`;

    // Generate steps from linked data
    for (const step of linkedData.generatedSteps) {
      test += this.generateTestStep(step, linkedData, _pageAnalysis);
    }

    // Add final assertions
    if (this.options.includeAssertions) {
      test += this.generateFinalAssertions(testCase, _pageAnalysis);
    }
  ...
  private generateTestStep(
    ...
    for (const [_inputId, inputData] of stepInputs) {
      stepCode += this.generateInputAction(inputData, _pageAnalysis);
    }
    ...
    // Add wait conditions
    stepCode += this.generateWaitConditions(step, _pageAnalysis);

    return stepCode;
  }

  private generateInputAction(inputData: any, _pageAnalysis: PageAnalysis): string {
  ...
  private generateWaitConditions(step: any, _pageAnalysis: PageAnalysis): string {
    let waitCode = '';
  ...
  private generateFinalAssertions(testCase: TestCase, pageAnalysis: PageAnalysis): string {
    ...
    if (testCase.test_data?.assertions && testCase.test_data.assertions.length > 0) {
      for (const assertion of testCase.test_data.assertions) {
        assertions += `    // ${assertion.description || assertion.type}\n`;
        assertions += `    cy.get('${assertion.target}').should('${assertion.type}', '${assertion.expected}');\n`;
      }
    }
  ...
  private generateHelperFunctions(_linkedData: LinkedTestData, _pageAnalysis: PageAnalysis): string {
    let helpers = '';

    // Generate form filling helper if multiple forms
    if (_pageAnalysis.forms.length > 1) {

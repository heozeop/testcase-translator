...
export const testCaseSchemas = {
  create: Joi.object({
    ...
    test_type: Joi.string().valid('functional', 'integration', 'regression', 'smoke').default('functional'),
  ...
  query: Joi.object({
    page: commonFields.page,
    limit: commonFields.limit,
    orderBy: Joi.string().valid('scenario_name', 'priority', 'status', 'created_at', 'updated_at').default('created_at'),
    order: commonFields.order,
    search: Joi.string().max(100).optional(),
    status: Joi.string().valid('draft', 'ready', 'processed', 'failed').optional(),
    priority: Joi.string().valid('low', 'medium', 'high', 'critical').optional(),
...
export const urlSchemas = {
  validate: Joi.object({
    ...
    options: Joi.object({
      ...
      maxSize: Joi.number().min(1024).max(10 * 1024 * 1024).default(5 * 1024 * 1024)
    }).default({})
  }),
...
export const inputSchemas = {
  request: Joi.object({
    ...
    input_requests: Joi.array().items(
      Joi.object({
        id: Joi.string().required(),
        type: Joi.string().valid('text', 'number', 'email', 'password', 'select', 'checkbox', 'radio', 'file').required(),
        prompt: Joi.string().min(1).max(500).required(),
        description: Joi.string().max(1000).optional(),
        required: Joi.boolean().default(true),
        category: Joi.string().valid('authentication', 'form-data', 'navigation', 'verification').required(),
        validationRules: Joi.array().items(
          Joi.object({
            type: Joi.string().valid('required', 'minLength', 'maxLength', 'pattern', 'email', 'url').required(),
...
export const searchSchemas = {
  global: Joi.object({
    ...
    types: Joi.array().items(
      Joi.string().valid('projects', 'test-cases', 'scripts')
...
export const batchSchemas = {
  operation: Joi.object({
    ...
    confirmation: Joi.boolean().default(false).when('operation', {
      is: 'delete',
...
export const systemSchemas = {
  settings: Joi.object({
    max_file_size: Joi.number().min(1024 * 1024).max(100 * 1024 * 1024).optional(), // 1MB to 100MB
